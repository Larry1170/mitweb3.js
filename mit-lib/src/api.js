const Api = provider => {
  ...
  const sendTransaction = send("mit_sendTransaction");
  const sendRawTransaction = send("mit_sendRawTransaction");
  const getTransactionReceipt = send("mit_getTransactionReceipt");
  const compileSolidity = send("mit_compileSolidity");
  const call = send("mit_call");
  const getBalance = send("mit_getBalance");
  const accounts = send("mit_accounts");
  ...
  const addTransactionDefaults = tx =>
    ...
    Promise.all([
      ...
      tx.gasPrice || send("mit_gasPrice")(),
      tx.nonce || send("mit_getTransactionCount")(tx.from,"latest"),
      tx.value || "0x0",
      tx.data || "0x"])
    // Add them to tx
    ...
    .then(tx => {
      ...
      if (!tx.gas) {
        ...
        return send("mit_estimateGas")(estimateTx)
          .then(usedGas => {
