...
var mitProvider = function mitProvider(url, intercept) {
  ...
  if (/^ws/.test(url)) {
    ...
    ws.on("open", function () {
      return callbacks.connect && callbacks.connect(mit);
    });
    ...
    api.send = makeSender(function (method, params, callback) {
      request(url, {
        ...
        return callback("Couldn't connect to mit node.");
      });
    });
