...
var Api = function Api(provider) {
  ...
  var encodeABI = function encodeABI(type, value) {
    if (type === "bytes") {
      ...
      throw "Mit-lib can't encode ABI type " + type + " yet.";
    }
  };

  var sendTransaction = send("mit_sendTransaction");
  var sendRawTransaction = send("mit_sendRawTransaction");
  var getTransactionReceipt = send("mit_getTransactionReceipt");
  var compileSolidity = send("mit_compileSolidity");
  var call = send("mit_call");
  var getBalance = send("mit_getBalance");
  var accounts = send("mit_accounts");
  ...
  var addTransactionDefaults = function addTransactionDefaults(tx) {
    return (
      // Get basic defaults
      Promise.all([tx.chainId || send("net_version")(), tx.gasPrice || send("mit_gasPrice")(), tx.nonce || send("mit_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"])
      // Add them to tx
      .then(function (_ref) {
        ...
        if (!tx.gas) {
          ...
          return send("mit_estimateGas")(estimateTx).then(function (usedGas) {
