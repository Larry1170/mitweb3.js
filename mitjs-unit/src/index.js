...
function toWei(tnbInput, unit) {
  ...
  if (tnb === '.') { throw new Error(`[mitjs-unit] while converting number ${tnbInput} to wei, invalid value`); }

  // Split it into a whole and fractional part
  var comps = tnb.split('.'); // eslint-disable-line
  if (comps.length > 2) { throw new Error(`[mitjs-unit] while converting number ${tnbInput} to wei,  too many decimal points`); }

  var whole = comps[0], fraction = comps[1]; // eslint-disable-line

  if (!whole) { whole = '0'; }
  if (!fraction) { fraction = '0'; }
  if (fraction.length > baseLength) { throw new Error(`[mitjs-unit] while converting number ${tnbInput} to wei, too many decimal places`); }
